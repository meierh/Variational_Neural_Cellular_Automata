...
input_name = '../weights/derma_hid_size_256.pth'
output_name_base = '../images/' + input_name.split('.')[-2].split('/')[-1] + '_process_image'
...
# Generate images using the model and save at intervals
all_images = []
with torch.no_grad():
    state = initial_state
...
# Stack all saved images into a single grid and save it
grid = make_grid(all_images, nrow=10, padding=2, normalize=True)
output_name = f'{output_name_base}_evolution.png'
save_image(grid, output_name)
print(f'Saved evolution image grid as {output_name}')
